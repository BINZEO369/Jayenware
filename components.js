...
function renderHeader() {
    ...
    <div class="mobile-menu-drawer" id="mobileMenuDrawer">
        ...
        <div class="mobile-menu-scroll" id="mobileMenuContent">
            ...
            <div class="mobile-cat-item" onclick="toggleMobileSubCategories()" style="border-bottom: none;">
...
function showToast(text, type = 'success') {
    ...
    if (type === 'success') {
        icon.className = 'w-8 h-8 rounded-full flex items-center justify-center text-sm bg-green-100 text-green-600 shrink-0';
        icon.innerHTML = '<i class="fa-solid fa-check"></i>';
    } else if (type === 'error') {
        icon.className = 'w-8 h-8 rounded-full flex items-center justify-center text-sm bg-red-100 text-red-600 shrink-0';
        icon.innerHTML = '<i class="fa-solid fa-exclamation"></i>';
    } else {
        icon.className = 'w-8 h-8 rounded-full flex items-center justify-center text-sm bg-blue-100 text-blue-600 shrink-0';
...
function addToCart(productId, productData) {
    ...
    if (cart.filter(i => i.product_id === productId).length >= product.stock) {
        return showToast('Stock limit reached', 'error');
    }
    cart.push({
        id: Date.now(),
...
function updateMobileSubCategories() {
    ...
    container.innerHTML = allCategories.map(c => 
        `<a href="/products?category=${c.name}" onclick="closeMobileMenu()" class="mobile-sub-cat">${c.name}</a>`
    ).join('');
}
...
async function initSharedComponents() {
    ...
    try {
        ...
        console.warn('Categories not loaded for mobile menu');
    }
    ...
    try {
        ...
        console.warn('Auth session not loaded');
    }
